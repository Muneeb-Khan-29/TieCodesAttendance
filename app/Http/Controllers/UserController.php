...
class UserController extends Controller
{
    ...
    {
        $totalHoursThisWeek = Attendance::whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
            ->sum('hours');

        $absenteesCount = Employee::whereNotIn('id', function ($query) {
        ...
        foreach ($daysOfWeek as $key => $day) {
            $attendanceRate[] = Attendance::whereDate('created_at', Carbon::now()->startOfWeek()->addDays($key))
                ->count();
        }

        return view('dashboard', compact('totalHoursThisWeek', 'absenteesCount', 'recentLogs', 'attendanceRate'));
    }
